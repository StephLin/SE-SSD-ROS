...
def one_hot_f(tensor, depth, dim=-1, on_value=1.0, dtype=torch.float32):
    ...
    tensor_onehot = torch.zeros(*list(tensor.shape),
                                depth,
                                dtype=dtype,
                                device=tensor.device)    # [4, 70400, 2]
    tensor_onehot.scatter_(dim,
                           tensor.unsqueeze(dim).long(),
                           on_value)    # [4, 70400, 2]
...
def add_sin_difference(boxes1, boxes2):
    rad_pred_encoding = torch.sin(boxes1[..., -1:]) * torch.cos(
        boxes2[..., -1:])    # ry -> sin(pred_ry)*cos(gt_ry)
    rad_gt_encoding = torch.cos(boxes1[..., -1:]) * torch.sin(
        boxes2[..., -1:])    # ry -> cos(pred_ry)*sin(gt_ry)
...
def get_direction_target(anchors, reg_targets, one_hot=True, dir_offset=0.0):
    ...
    rot_gt = reg_targets[..., -1] + anchors[...,
                                            -1]    # original ry, [4, 70400]
    dir_cls_targets = (
        (rot_gt - dir_offset) >
        0).long()    # [4, 70400], elem: 0 or 1, todo: physical scene
    if one_hot:
...
def smooth_l1_loss(pred, gt, sigma):
    def _smooth_l1_loss(pred, gt, sigma):
        ...
        value = in_value * in_mask.type_as(
            in_value) + out_value * out_mask.type_as(out_value)
        return value
...
def smooth_l1_loss_detectron2(input,
                              target,
                              beta: float,
                              reduction: str = "none"):
    """
...
def create_loss(
    ...
    box_preds = box_preds.view(
        batch_size, -1, box_code_size)    # [8, 200, 176, 14] -> [8, 70400, 7]
    cls_preds = cls_preds.view(batch_size, -1,
                               num_class)    # [8, 70400] -> [8, 70400, 1]
    ...
    cls_losses = cls_loss_ftor(cls_preds, cls_targets,
...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.trans_conv:
            x = self.trans_conv(x)
        box_preds = self.conv_box(x).permute(
            0, 2, 3, 1).contiguous()    # box_preds.shape=[8, 200, 176, 14]
        cls_preds = self.conv_cls(x).permute(
            0, 2, 3, 1).contiguous()    # cls_preds.shape=[8, 200, 176, 2]
        ret_dict = {"box_preds": box_preds, "cls_preds": cls_preds}
        if self.use_dir:
            dir_preds = self.conv_dir(x).permute(
                0, 2, 3, 1).contiguous()    # dir_preds.shape=[8, 200, 176, 4]
            ret_dict["dir_cls_preds"] = dir_preds

        ret_dict["iou_preds"] = self.conv_iou(x).permute(0, 2, 3,
                                                         1).contiguous()
...
class RegHead(nn.Module):
    ...
    def loss(self, example, preds, **kwargs):
        ...
        for task_id, task_pred in enumerate(preds):
            ...
            h_intersect = torch.min(yp_top, yg_top) - torch.max(
                yp_down, yg_down)
...
class MultiGroupHead(nn.Module):
    def __init__(
        ...
        num_preds = []
        num_dirs = []
        for num_c, num_a, box_cs in zip(num_classes, self.num_anchor_per_locs,
                                        box_code_sizes):    # 1, 2, 7
        ...
        self.tasks = nn.ModuleList()
        for task_id, (num_pred, num_cls) in enumerate(zip(num_preds,
                                                          num_clss)):
        ...
        self.top_labels = torch.zeros(
            [70400],
            dtype=torch.long,
        ).cuda()    # [70400]
        self.loss_size_consistency = nn.MSELoss(reduction='mean')
        self.loss_iou_consistency = build_loss(
            dict(
                type="WeightedSmoothL1Loss",
                sigma=3.0,
                code_weights=None,
        ...
        self.loss_score_consistency = build_loss(
            dict(
                type="WeightedSmoothL1Loss",
                sigma=3.0,
                code_weights=None,
    ...
    def prepare_loss_weights(
        ...
        '''
        loss_norm_type = getattr(LossNormType,
                                 loss_norm["type"])    # norm_by_num_positives
        ...
        if loss_norm_type == LossNormType.NormByNumExamples:
            ...
            reg_weights /= torch.clamp(
                pos_normalizer,
                min=1.0)    # [N, num_anchors], average in each sample;
            cls_weights /= torch.clamp(
                pos_normalizer, min=1.0
            )    # todo: interesting, how about the negatives samples
    ...
    def nn_distance(self, box1, box2, iou_thres=0.7, return_loss='10'):
        ...
        if ans_iou.shape[0] == 0 or ans_iou.shape[
                1] == 0:    # for unlabeled data (some scenes wo cars)
            return [None] * 5
        ...
        encoded_box_preds, encoded_reg_targets = add_sin_difference(
            val_box1, aligned_box2)
        loss1 = self.loss_reg(encoded_box_preds,
                              encoded_reg_targets).sum(-1) / 7.
        encoded_box_preds, encoded_reg_targets = add_sin_difference(
            val_box2, aligned_box1)
        loss2 = self.loss_reg(encoded_box_preds,
                              encoded_reg_targets).sum(-1) / 7.
        if return_loss == '10':
            ...
            box_consistency_loss = (loss1.sum() + loss2.sum()) / (
                loss1.shape[0] + loss2.shape[0])
        else:
    ...
    def per_box_loc_trans(self, boxes, gt_boxes, trans):
        gt_indices = iou3d_utils.boxes_iou3d_gpu(
            boxes.cuda(),
            torch.from_numpy(gt_boxes).float().cuda()).max(-1)[1]
        trans_loc = torch.from_numpy(
            trans['translation_loc']).float().cuda()[gt_indices]
        rot_loc = torch.from_numpy(
            trans['rotation_loc']).float().cuda()[gt_indices]
        boxes[:, :3] += trans_loc
        boxes[:, 6] += rot_loc
        return boxes
    ...
    def consistency_loss(self, preds_stu, preds_tea, example):
        ...
        batch_dir_preds_stu = preds_stu[0]["dir_cls_preds"].view(
            batch_size, -1, 2)
        ...
        batch_dir_preds_tea = preds_tea[0]["dir_cls_preds"].view(
            batch_size, -1, 2)
        ...
        for box_preds_stu_offset, cls_preds_stu, dir_preds_stu, iou_preds_stu, \
            ...
            batch_id += 1
            box_preds_stu = self.box_coder.decode_torch(
                box_preds_stu_offset, example["anchors"][0][0])
            box_preds_tea = self.box_coder.decode_torch(
                box_preds_tea_offset, example["anchors"][0][0])

            # filter predicted boxes
            top_scores_keep_stu = torch.sigmoid(cls_preds_stu).squeeze(
                -1) >= 0.3    # [70400]
            top_scores_keep_tea = torch.sigmoid(cls_preds_tea).squeeze(
                -1) >= 0.3    # [70400]
            pos_anchors = example['anchors'][0][0][top_scores_keep_tea]
            mask_stu = (box_preds_stu[:, :3] >=
                        self.post_center_range[:3]).all(1)
            mask_stu &= (box_preds_stu[:, :3] <=
                         self.post_center_range[3:]).all(1)
            mask_stu &= top_scores_keep_stu
            mask_tea = (box_preds_tea[:, :3] >=
                        self.post_center_range[:3]).all(1)
            mask_tea &= (box_preds_tea[:, :3] <=
                         self.post_center_range[3:]).all(1)
            ...
            if mask_stu.sum() > 0 and mask_tea.sum() > 0:
                ...
                top_box_preds_tea[:, 1] = -top_box_preds_tea[:, 1] if trans[
                    "flipped"] else top_box_preds_tea[:, 1]
                top_box_preds_tea[:,
                                  -1] = -top_box_preds_tea[:, -1] + np.pi if trans[
                                      "flipped"] else top_box_preds_tea[:, -1]
                top_box_preds_tea[:, :
                                  3] = box_torch_ops.rotation_points_single_angle(
                                      top_box_preds_tea[:, :3],
                                      trans["noise_rotation"],
                                      axis=2)
                ...
                box_consistency_loss, idx1, idx2, mask1, mask2 = self.nn_distance(
                    top_box_preds_stu, top_box_preds_tea)
                if box_consistency_loss is None:
                    continue
                batch_box_loss += box_consistency_loss

                # cls_score consistency loss
                aligned_cls_preds_stu, aligned_cls_preds_tea = top_cls_preds_stu[
                    mask1][idx2], top_cls_preds_tea[mask2][idx1]
                scores_stu, scores_tea = torch.sigmoid(
                    top_cls_preds_stu[mask1]), torch.sigmoid(
                        aligned_cls_preds_tea)
                score_consistency_loss = self.loss_score_consistency(
                    scores_stu, scores_tea).mean()
                batch_cls_loss += score_consistency_loss

                # iou consistency loss
                aligned_iou_preds_tea = (top_iou_preds_tea[mask2][idx1] +
                                         1) * 0.5
                top_iou_preds_stu = (top_iou_preds_stu[mask1] + 1) * 0.5
                iou_consistency_loss = self.loss_iou_consistency(
                    top_iou_preds_stu, aligned_iou_preds_tea).mean()
                batch_iou_loss += iou_consistency_loss

                # dir consistency loss
                aligned_dir_preds_tea = top_dir_preds_tea[mask2][idx1]
                aligned_dir_preds_tea = F.softmax(aligned_dir_preds_tea,
                                                  dim=-1)
                top_dir_preds_stu = F.softmax(top_dir_preds_stu[mask1], dim=-1)
                dir_consistency_loss = self.loss_dir_consistency(
                    top_dir_preds_stu, aligned_dir_preds_tea)
                batch_dir_loss += dir_consistency_loss

        consistency_loss = (1.0 * batch_box_loss + 1.0 * batch_cls_loss +
                            1.0 * batch_iou_loss) / batch_size
        return consistency_loss

    def loss(self, example, preds_dicts, preds_ema, **kwargs):
        supervision_mask = example[
            "ssl_labeled"] == 1 if "ssl_labeled" in example.keys(
            ) else torch.ones(len(example['metadata'])) == 1
        consistency_loss = self.consistency_loss(preds_dicts, preds_ema,
                                                 example)
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            labels = example["labels"][
                task_id]    # cls_labels: [batch_size, 70400], elem in [-1, 0, 1].
            reg_targets = example["reg_targets"][
                task_id]    # reg_labels: [batch_size, 70400, 7].
            cls_weights, reg_weights, cared = self.prepare_loss_weights(
                labels,
                loss_norm=self.loss_norm,
                dtype=torch.float32,
            )    # all: [batch_size, 70400]
            cls_targets = labels * cared.type_as(
                labels)    # filter -1 in labels.
            cls_targets = cls_targets.unsqueeze(
                -1)    # [batch_size, 70400, 1].

            # get localization and classification loss.
            batch_size = int(box_preds.shape[0])
            box_preds = box_preds.view(
                batch_size, -1, self.box_n_dim
            )    # [batch_size, 200, 176, 14] -> [batch_size, 70400, 7].
            cls_preds = cls_preds.view(
                batch_size, -1, self.num_classes[task_id]
            )    # [batch_size, 70400] -> [batch_size, 70400, 1].
            ...
            loc_loss = self.loss_reg(
                encoded_box_preds, encoded_reg_targets, weights=reg_weights
            )    # [N, 70400, 7], WeightedSmoothL1Loss, averaged in sample.
            cls_loss = self.loss_cls(
                cls_preds, cls_targets, weights=cls_weights
            )    # [N, 70400, 1], SigmoidFocalLoss, averaged in sample.
            loc_loss_reduced = self.loss_reg._loss_weight * loc_loss.sum(
            ) / batch_size_device    # 2.0, averaged on batch_size
            cls_loss_reduced = self.loss_cls._loss_weight * cls_loss.sum(
            ) / batch_size_device    # 1.0, average on batch_size

            cls_pos_loss, cls_neg_loss = _get_pos_neg_loss(
                cls_loss, labels)    # for analysis, average on batch
            ...
            if self.use_direction_classifier:    # True
                dir_targets = get_direction_target(
                    example["anchors"][task_id][supervision_mask],
                    reg_targets,
                    dir_offset=self.direction_offset,
                )    # [8, 70400, 2]
                dir_logits = preds_dict["dir_cls_preds"][supervision_mask].view(
                    batch_size_device, -1,
                    2)    # [8, 70400, 2], WeightedSoftmaxClassificationLoss.
                weights = (labels > 0).type_as(
                    dir_logits)    # [8, 70400], only for positive anchors.
                weights /= torch.clamp(
                    weights.sum(-1, keepdim=True),
                    min=1.0)    # [8, 70400], averaged in sample.
                dir_loss = self.loss_aux(dir_logits,
                                         dir_targets,
                                         weights=weights)    # [8, 70400].
                dir_loss = self.loss_aux._loss_weight * dir_loss.sum(
                ) / batch_size_device    # averaged in batch.
            ...
            qboxes = self.box_coder.decode_torch(
                box_preds[pos_pred_mask],
                example["anchors"][0][supervision_mask][pos_pred_mask])
            gboxes = self.box_coder.decode_torch(
                reg_targets[pos_pred_mask],
                example["anchors"][0][supervision_mask][pos_pred_mask])
            iou_weights = reg_weights[pos_pred_mask]
            iou_pos_targets = iou3d_utils.boxes_aligned_iou3d_gpu(
                qboxes, gboxes).detach()
            iou_pos_targets = 2 * iou_pos_targets - 1
            iou_pred_loss = self.loss_iou_pred(iou_pos_preds, iou_pos_targets,
                                               iou_weights)
            ...
            if pos_pred_mask.sum() > 0:
                qboxes = self.box_coder.decode_torch(preds_dict["box_preds"][supervision_mask].view(batch_size, -1, 7)[pos_pred_mask], \
                                                     example["anchors"][0][supervision_mask][pos_pred_mask])
                gboxes = self.box_coder.decode_torch(
                    example["reg_targets"][0][pos_pred_mask],
                    example["anchors"][0][supervision_mask][pos_pred_mask])
                weights = reg_weights[pos_pred_mask]
                ious_loss = self.odiou_3d_loss(gboxes, qboxes, weights,
                                               batch_size)
            ...
            ret = {
                ...
                loss,
                "cls_loss_reduced":
                cls_loss_reduced.detach().cpu().mean(),
                "loc_loss_reduced":
                loc_loss_reduced.detach().cpu().mean(),
                "dir_loss_reduced":
                dir_loss.detach().cpu()
                if self.use_direction_classifier else None,
                "iou_pred_loss":
                iou_pred_loss.detach().cpu(),
                "consistency_loss":
                consistency_loss,
                "loc_loss_elem":
                [elem.detach().cpu() for elem in loc_loss_elem],
                "cls_pos_loss":
                cls_pos_loss.detach().cpu(),
                "cls_neg_loss":
                cls_neg_loss.detach().cpu(),
                "ious_loss":
                ious_loss.detach().cpu(),
    ...
    def get_model_ema_loss(self, example, preds_dicts):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            loc_loss = self.loss_reg(encoded_box_preds,
                                     encoded_reg_targets,
                                     weights=reg_weights)
            cls_loss = self.loss_cls(cls_preds,
                                     cls_targets,
                                     weights=cls_weights)
            loc_loss_reduced = self.loss_reg._loss_weight * loc_loss.sum(
            ) / batch_size_device
            ...
            if self.use_direction_classifier:    # True
                dir_targets = get_direction_target(
                    example["anchors_raw"][task_id][supervision_mask],
                    reg_targets,
                    dir_offset=self.direction_offset,
                )
                dir_logits = preds_dict["dir_cls_preds"][
                    supervision_mask].view(batch_size_device, -1, 2)
            ...
            qboxes = self.box_coder.decode_torch(
                box_preds[pos_pred_mask],
                example["anchors_raw"][0][supervision_mask][pos_pred_mask])
            gboxes = self.box_coder.decode_torch(
                reg_targets[pos_pred_mask],
                example["anchors_raw"][0][supervision_mask][pos_pred_mask])
            iou_weights = reg_weights[pos_pred_mask]
            ...
            ret = {
                "loss_ema":
                loss.detach().cpu(),
                "cls_loss_reduced_ema":
                cls_loss_reduced.detach().cpu().mean(),
                "loc_loss_reduced_ema":
                loc_loss_reduced.detach().cpu().mean(),
                "dir_loss_reduced_ema":
                dir_loss.detach().cpu()
                if self.use_direction_classifier else None,
                "iou_pred_loss_ema":
                iou_pred_loss.detach().cpu(),
    ...
    def predict(self, example, preds_dicts, test_cfg, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            batch_task_anchors = example["anchors"][task_id].view(
                batch_size, -1, self.box_n_dim)    # [8, 70400, 7]
            batch_anchors_mask = [None] * batch_size
            batch_cls_preds = preds_dict["cls_preds"].view(
                batch_size, -1, num_class_with_bg)    # [8, 70400, 1]
            batch_box_preds = preds_dict["box_preds"].view(
                batch_size, -1, self.box_n_dim)    # [batch_size, 70400, 7]
            ...
            batch_reg_preds = self.box_coder.decode_torch(
                ...
                batch_task_anchors)
            batch_dir_preds = preds_dict["dir_cls_preds"].view(
                batch_size, -1, 2)    # [8, 200, 176, 4]) -> [8, 70400, 2]

            rets.append(
                self.get_task_detections(task_id, num_class_with_bg, test_cfg,
                                         batch_cls_preds, batch_reg_preds,
                                         batch_dir_preds, batch_iou_preds,
                                         batch_anchors, batch_anchors_mask,
                                         meta_list))
        num_samples = len(rets[0])
    ...
    def get_task_detections(self,
        ...
        for box_preds, cls_preds, dir_preds, iou_preds, a_mask, meta in zip(
                batch_reg_preds, batch_cls_preds, batch_dir_preds,
                batch_iou_preds, batch_anchors_mask, meta_list):
            # get dir labels
            if self.use_direction_classifier:
            ...
            if test_cfg.score_threshold > 0.0:
                ...
                top_scores *= torch.pow(
                    iou_preds.masked_select(top_scores_keep), 4)
            ...
            if top_scores.shape[0] != 0:
                ...
                # Still add DI-NMS in CIA-SSD to SE-SSD due to its minor positive effect.
                    ...
                    box_preds, dir_labels, label_preds, scores, selected = nms_func(
                        box_preds,
                        boxes_for_nms,
                        dir_labels,
                        top_labels,
            ...
            #         box_preds[:, :3].cpu().numpy(),
            #         valid_frustum.cpu().numpy())
            ...
            if box_preds.shape[0] != 0:
                ...
                if self.use_direction_classifier:
                    opp_labels = ((box_preds[..., -1] - self.direction_offset)
                                  > 0) ^ (dir_labels.byte() == 1)
                    box_preds[..., -1] += torch.where(
                        opp_labels,
                        torch.tensor(np.pi).type_as(box_preds),
                        torch.tensor(0.0).type_as(box_preds),
                    )    # useful for dir accuracy, but has no impact on localization
